...
import secrets  # Use this for cryptographically secure randomness
...
def EG_generate_keys():
    # Use a cryptographically secure random integer in [1..(PARAM_Q-1)]
    private_key = secrets.randbelow(PARAM_Q - 1) + 1
    public_key = pow(PARAM_G, private_key, PARAM_P)
    return private_key, public_key

def EGM_encrypt(message, public_key):
    # Use a cryptographically secure random integer in [1..(PARAM_Q-1)]
    k = secrets.randbelow(PARAM_Q - 1) + 1
